...
def get_response(image_data: str) -> str:
    ...
    try:
        ...
        logger.error(f"Error getting response for image: {e}")
        return ""
...
def process_screenshot(screenshot_id):
    ...
    try:
        ...
        logger.error(f"Error converting image for screenshot {screenshot_id}: {e}")
    ...
    if description:
        supabase.table("screenshots").update(
            {"description": description, "status": "vision_done"}
